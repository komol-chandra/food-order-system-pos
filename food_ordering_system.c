    ...
    do
{
	...
	{
	printf("  \n\n\n       WELCOME TO FOOD ORDER SYSTEM (POS) !!!! LOGIN IS SUCCESSFUL");
...
void d_mainmenu()
{
    int i;
    char ch;
    const char *menu[]= {"   Calculate Bill","   Add Items","   Display Items","  Exit   "};
    system("cls");
    window(25,50,20,32);
    gotoxy(33,18);
    printf("POS MENU");
...
//textcolor(0);
    ...
    while(toupper(getche())=='Y')
    {
        ...
        fwrite(&item,sizeof(item),1,file);//need to konw
        fclose(file);
        gotoxy(22,34);
        printf("Enter New Record(Y/N)?");
...
/* ---#k function to check availability of code*/
void c_code(char y[])
{
    int flag;
    FILE *file;
...
//textcolor(1);
    for (i=1; i<=10; i++)
        printf("\xdb");
    printf(" WELCOME TO FOOD ORDER SYSTEM (POS) ");
    for (i=1; i<=10; i++)
        printf("\xdb");
    printf("\n\n");
    gotoxy(30,11);
